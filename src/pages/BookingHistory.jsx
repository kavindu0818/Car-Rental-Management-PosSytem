...
const BookingHistory = () => {
  ...
  const bookingHistory = useSelector(state => state.bookingHistory?.bookingHistory || []);
  const navigate = useNavigate(); // Initialize navigate function

  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    dispatch(getAllBookingHistory());
  }, [dispatch]);
  ...
  console.log(sortedBookings); // Fixed undefined variable
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  {sortedBookings.map((booking, index) => (
                      <tr key={booking.bookingId || booking.id || index}> {/* Ensuring unique key */}
